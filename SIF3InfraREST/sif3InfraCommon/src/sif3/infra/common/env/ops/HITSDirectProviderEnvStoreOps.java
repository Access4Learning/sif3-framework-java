/*
 * HITSDirectProviderEnvStoreOps.java
 * Created: 20/02/2014
 *
 ...
 *
 */
public class HITSDirectProviderEnvStoreOps extends AdapterBaseEnvStoreOperations
{
     ...
     */
    public HITSDirectProviderEnvStoreOps(String adapterFileNameWithoutExt)
    {
...
//	      	logger.error("The application key and/or the solution id in the consumer input environment is null or empty. Environment cannot be created.");
		...
		try
		{
			...
			updateTemplateInfo(environment, envKey);
	...
	/*
	 * This is a HITS Custom operation.
	 */
	private void updateTemplateInfo(EnvironmentType templEnv, EnvironmentKey envKey)
	{
		// Set the default Zone to userToken.
		templEnv.getDefaultZone().setId(envKey.getUserToken());
		
		// Search for "HITS" zone in the provisioned zone list and replace it with userToken
		for (ProvisionedZoneType zone : templEnv.getProvisionedZones().getProvisionedZone())
		{
			if (zone.getId().equals(HITSConstants.HITS_ZONE_TAG))
			{
				zone.setId(envKey.getUserToken());
				break;
			}
		}
	}
