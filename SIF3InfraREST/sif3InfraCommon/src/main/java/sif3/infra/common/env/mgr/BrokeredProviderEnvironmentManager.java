/*
 * BrokeredProviderEnvironmentManager.java
 * Created: 15/03/2014
 *
...
import sif3.infra.common.env.ops.BrokeredProviderEnvStoreOps;
 ...
 * a brokered environment). A provider connects, creates or removes environments. This functionality must be maintained in a provider 
 * but also communicated to the environment provider (i.e. broker). This class ensures that these two components are kept in sync 
 * and communicate appropriate actions with each other. It is not expected that high level classes of this framework would interact 
 * with the environment manager directly.
 * 
 * @author Joerg Huber
 */
public class BrokeredProviderEnvironmentManager implements ClientEnvironmentManager
{
	...
	private BrokeredProviderEnvStoreOps envOps = null;

	/* Active SIF3 Session of this consumer. There is only one. */
	private SIF3Session sif3Session = null;

	private static BrokeredProviderEnvironmentManager instance = null;
	 ...
	 */
	public static BrokeredProviderEnvironmentManager initialse(String adapterFileNameWithoutExt)
	{
		if (instance == null)
		{
			instance = new BrokeredProviderEnvironmentManager(adapterFileNameWithoutExt);
		}
	 ...
	 * @return See Desc.
	 */
	public static BrokeredProviderEnvironmentManager getInstance()
	{
		return instance;
	}
    ...
    public SIF3Session getSessionBySessionToken(String sessionToken)
    {
	    if ((getSIF3Session() != null) && (StringUtils.notEmpty(sessionToken)))
	    {
	    	...
	    	{
	    		logger.error("The sessionToken for the session in this Provider Environment Manager ("+getSIF3Session().getSessionToken()+") is different to the sessionToken from the request (" + sessionToken +"). User is not allowed to access this session.");
	    		return null;
	    	}
	    }
    ...
    public SIF3Session getSessionBySecurityToken(String securityToken)
    {
      if ((getSIF3Session() != null) && (StringUtils.notEmpty(securityToken)))
      {
        ...
        {
          logger.error("The securityToken for the session in this Provider Environment Manager ("+getSIF3Session().getSecurityToken()+") is different to the securityToken from the request (" + securityToken +"). User is not allowed to access this session.");
          return null;
        }
      }
      return null; // no session or invalid sessionToken
	 ...
	 */
	private BrokeredProviderEnvironmentManager(String adapterFileNameWithoutExt)
	{
		super();
	...
	private ProviderEnvironment getProviderEnvrionment()
	{
		return (ProviderEnvironment)getEnvironmentInfo();
	}
