...
public class LearnerPersonalIterator implements SIFEventIterator<LearnerPersonalCollectionType>
{
    ...
    public LearnerPersonalIterator(String providerID, AdvancedProperties serviceProperties,
            HashMap<String, LearnerPersonalType> students)
    {
        currentPos = 0;
        if (students != null)
        {
    ...
    public SIFEvent<LearnerPersonalCollectionType> getNextEvents(int maxListSize)
    {
        ...
        if (hasNext())
        {
            events = new SIFEvent<LearnerPersonalCollectionType>(
                    new LearnerPersonalCollectionType(), EventAction.UPDATE, UpdateType.FULL, 0);
