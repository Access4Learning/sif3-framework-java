/*
 * LearnerPersonalProvider.java Created: 01/10/2013 Copyright 2013 Systemic Pty
...
public class LearnerPersonalProvider extends UKDataModelProviderWithEvents<LearnerPersonalCollectionType> implements QueryProvider, ChangesSinceProvider {
	...
	public LearnerPersonalProvider() {
		...
		if (learners == null) {
			logger.debug("Constructor for LearnerPersonalProvider called for the first time. Try to load Learners from XML file...");

			learners = new HashMap<String, LearnerPersonalType>();
			
			// Load all Learners so that we can do some real stuff here.
			String learnerFile = getServiceProperties().getPropertyAsString("provider.learner.file.location", null);
			if (learnerFile != null) {
				String inputXML = FileReaderWriter.getFileContent(learnerFile, ModelObjectConstants.UTF_8);
				try {
					LearnerPersonalCollectionType learnerList = (LearnerPersonalCollectionType) getUnmarshaller().unmarshalFromXML(inputXML, getMultiObjectClassInfo().getObjectType());
	...
	/*
	 * (non-Javadoc)
	 * @see sif3.common.interfaces.EventProvider#onEventError(sif3.common.model.
	 * SIFEvent, sif3.common.model.SIFZone, sif3.common.model.SIFContext)
	 */
	@Override
	public void onEventError(SIFEvent<LearnerPersonalCollectionType> sifEvent, SIFZone zone, SIFContext context) {
	 ...
	 */
	@Override
	public SIFEvent<LearnerPersonalCollectionType> modifyBeforePublishing(SIFEvent<LearnerPersonalCollectionType> sifEvent, SIFZone zone, SIFContext context, HeaderProperties customHTTPHeaders) {
	...
	public Object retrieveByPrimaryKey(String resourceID, SIFZone zone, SIFContext context, RequestMetadata metadata) throws IllegalArgumentException, PersistenceException {
		...
		logger.debug("Retrieve learner with Resource ID = " + resourceID + ", " + getZoneAndContext(zone, context) + " and RequestMetadata = " + metadata);
	...
	public Object createSingle(Object data, boolean useAdvisory, SIFZone zone, SIFContext context, RequestMetadata metadata) throws IllegalArgumentException, PersistenceException {
		...
		if (data instanceof LearnerPersonalType) {
			...
			throw new IllegalArgumentException("Expected Object Type  = LearnerPersonalType. Received Object Type = " + data.getClass().getSimpleName());
		}
	}
	...
	public boolean updateSingle(Object data, String resourceID, SIFZone zone, SIFContext context, RequestMetadata metadata) throws IllegalArgumentException, PersistenceException {
		...
		// Must be of type LearnerPersonalType
		if (data instanceof LearnerPersonalType) {
			logger.debug("Update learner with Resoucre ID = " + resourceID + ", " + getZoneAndContext(zone, context) + " and RequestMetadata = " + metadata);
	...
	public boolean deleteSingle(String resourceID, SIFZone zone, SIFContext context, RequestMetadata metadata) throws IllegalArgumentException, PersistenceException {
		...
		logger.debug("Remove learner with Resoucre ID = " + resourceID + ", " + getZoneAndContext(zone, context) + " and RequestMetadata = " + metadata);
	...
	public Object retrieve(SIFZone zone, SIFContext context, PagingInfo pagingInfo, RequestMetadata metadata) throws PersistenceException, UnsupportedQueryException, DataTooLargeException {
		...
		LearnerPersonalCollectionType LearnerCollection = dmObjectFactory.createLearnerPersonalCollectionType();
	 ...
	 * (non-Javadoc)
	 * @see
	 * sif3.common.interfaces.QueryProvider#retrieveByServicePath(sif3.common.
	 * model.QueryCriteria, sif3.common.model.SIFZone,
	 * sif3.common.model.SIFContext, sif3.common.model.PagingInfo,
	...
	public Object retrieveByQBE(Object exampleObject, SIFZone zone, SIFContext context, PagingInfo pagingInfo, RequestMetadata metadata) throws PersistenceException, UnsupportedQueryException, DataTooLargeException {
		...
		if (exampleObject instanceof LearnerPersonalType) {
			...
			throw new IllegalArgumentException("Expected Object Type  = LearnerPersonalType. Received Object Type = " + exampleObject.getClass().getSimpleName());
		}
		...
		if (data instanceof LearnerPersonalCollectionType) {
			...
			for (LearnerPersonalType learner : learners.getLearnerPersonal()) {
				if ((i % 3) == 0) {
					...
					opStatus.add(new CreateOperationStatus(learner.getRefId(), learner.getRefId(), 404, new ErrorDetails(400, "Data not good.")));
				} else {
					if (useAdvisory) {
						// Advisory refId was used. Set resourceId and
						// advisoryId to the same
						opStatus.add(new CreateOperationStatus(learner.getRefId(), learner.getRefId(), 201));
					} else {
						// Create a new refId (resourceID) but we must also
						// report back the original RefId as the advisory if it
						// was available.
						opStatus.add(new CreateOperationStatus(UUIDGenerator.getSIF2GUIDUpperCase(), learner.getRefId(), 201));
					}
				}
				i++;
			}
			...
			throw new IllegalArgumentException("Expected Object Type  = LearnerCollectionType. Received Object Type = " + data.getClass().getSimpleName());
		}
	}
	...
	public List<OperationStatus> updateMany(Object data, SIFZone zone, SIFContext context, RequestMetadata metadata) throws IllegalArgumentException, PersistenceException {
		...
		if (data instanceof LearnerPersonalCollectionType) {
			...
			for (LearnerPersonalType learner : learners.getLearnerPersonal()) {
				if ((i % 3) == 0) {
					opStatus.add(new OperationStatus(learner.getRefId(), 404, new ErrorDetails(404, "Learner with GUID = " + learner.getRefId() + " does not exist.")));
				} else {
	...
	public List<OperationStatus> deleteMany(List<String> resourceIDs, SIFZone zone, SIFContext context, RequestMetadata metadata) throws IllegalArgumentException, PersistenceException {
		...
		for (String resourceID : resourceIDs) {
			if ((i % 3) == 0) {
				opStatus.add(new OperationStatus(resourceID, 404, new ErrorDetails(404, "Learner with GUID = " + resourceID + " does not exist.")));
			} else {
	...
	/*
	 * Override the method from the AUDataModelProviderWithEvents class and set a
	 * few custom headers. (non-Javadoc)
	 * @see systemic.sif3.demo.rest.provider.AUDataModelProviderWithEvents#
	 * getCustomServiceInfo(sif3.common.model.SIFZone,
	 * sif3.common.model.SIFContext, sif3.common.model.PagingInfo,
	 * sif3.common.model.RequestMetadata)
	 */
	@Override
	...
	/*
	 * For the purpose of illustrating the use of the changes since functionality
	 * we assume that changes since are provided by this Object provider based on
	 * date time stamps. Real implementations may look up a table to get the
	 * latest value of the opaque marker. (non-Javadoc)
	 ...
	 * (non-Javadoc)
	 * @see
	 * sif3.common.interfaces.ChangesSinceProvider#getChangesSince(sif3.common.
	 * model.SIFZone, sif3.common.model.SIFContext, sif3.common.model.PagingInfo,
	...
	private ArrayList<LearnerPersonalType> fetchLearners(HashMap<String, LearnerPersonalType> LearnerMap, PagingInfo pagingInfo) {
