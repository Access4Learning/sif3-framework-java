...
public abstract class UKDataModelEventConsumer<L> extends AbstractEventConsumer<L>
{
    ...
    public UKDataModelEventConsumer()
    {
        super();
        if (busyInSec == null)
        {
        ...
        {
            writePayload = getServiceProperties().getPropertyAsBool("test.consumer.write.payload",
                    true);
        }

        // Initialise JAXB context for these classes. Make data processor behave
     ...
     */
    @Override
    public void processEvent(SIFEvent<L> sifEvent, SIFZone zone, SIFContext context,
            EventMetadata metadata, String msgReadID, String consumerID)
    {
        String consumerName = getPrettyName() + "(QueueID:" + msgReadID + "; ConsumerID: "
                + consumerID + ")";

        // We know from the framework that zone and context is never null. For
        // the time being we just log the event.
        logger.debug(consumerName + " received an event from Zone = " + zone.getId()
    ...
    private void dumpObject(SIFEvent<L> sifEvent, SIFZone zone, SIFContext context,
            String msgReadID, String consumerID)
    {
        ...
        try
        {
            ...
            out.write("Record " + recordNum + " - processed by Thread ID = "
                    + Thread.currentThread().getId() + "\n" + sifEvent.getListSize() + " "
                    + sifEvent.getEventAction().name() + " Events from Queue Reader " + msgReadID
                    + "\nReceived at " + timestamp + " from Zone = " + zone.getId()
                    + " and Context = " + context.getId());
            out.write(RECORD_MARKER);
            if (writePayload)
            {
                out.write(getMarshaller().marshal(sifEvent.getSIFObjectList(),
    ...
    {
        return getServiceProperties().getPropertyAsString("test.tempDir.output", "") + "/"
                + getFileNameOnly(consumerID);
