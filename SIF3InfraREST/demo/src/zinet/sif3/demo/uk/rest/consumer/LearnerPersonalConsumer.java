...
public class LearnerPersonalConsumer extends UKDataModelEventConsumer<LearnerPersonalCollectionType> {
		...
		new Timer().schedule(new TimerTask() {
			...
			public void run() {
				...
				try {
					...
					logger.info("Fetched " + name.getGivenName().getValue() + " " + name.getFamilyName().getValue());
	 ...
	 */
	@Override
	public SIFEvent<LearnerPersonalCollectionType> createEventObject(Object sifObjectList, EventAction eventAction, UpdateType updateType) {
		if (sifObjectList != null) {
			if (sifObjectList instanceof LearnerPersonalCollectionType) {
				int size = ((LearnerPersonalCollectionType) sifObjectList).getLearnerPersonal().size();
				return new SIFEvent<LearnerPersonalCollectionType>((LearnerPersonalCollectionType) sifObjectList, eventAction, updateType, size);
			} else {
				logger.error("The given event data is not of type LearnerCollectionType as expected. Cannot create event object. Return null");
			}
		} else {
