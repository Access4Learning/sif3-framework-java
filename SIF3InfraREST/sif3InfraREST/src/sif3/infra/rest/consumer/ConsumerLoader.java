...
/**
 * This is the initialise/finalise class that must be called at startup or shutdown of any consumer
 * adapter/service. The methods in here are only required to be called once. In fact the singleton
 * nature of this class will ensure that methods can only be called once.
 * 
 * The purpose of this class is to initialise a consumer at startup. This includes but is not
 * limited to:<br/>
 ...
 * <br/>
 * 
 * Once consumer is shutdown the finalise method of this class should be called to free up all
 * allocated resources and shut down related components.
 * 
 * @author Joerg Huber
 */
public class ConsumerLoader
{
    protected static final Logger          logger                = Logger
            .getLogger(ConsumerLoader.class);
    ...
    /**
     * Initialises the consumer based on the given property file. If anything fails to initialise
     * then an error is logged and this method returns false. The consumer should not really
     * continue in such a case as its behaviour is not defined and most likely will throw exceptions
     * in many places.
     * 
    ...
    /**
     * There are some places in the code where we assume that the initialise of this method has been
     * called. Where this is the case this method can be called to ensure that indeed the initialise
     * has been called. If the initialise isn't called then it is really a programming error rather
     * than a runtime error. In many cases the consumer should shut down if the initialise is not
     * called successfully.
     * 
    ...
    private ConsumerLoader(String consumerPropertyFileName) throws Exception
    {
        if (StringUtils.isEmpty(consumerPropertyFileName))
        {
            logAndThrowException(
                    "Consumer Property File name is null or empty. Cannot initialse consumer.");
        }
        ...
        // Check if we need to initialise the consumer properties. This should only happen once.
        logger.debug("Initialise Consumer Environment Manager and Session Store with property file "
                + consumerPropertyFileName + ".properties");
        ConsumerEnvironmentManager envMgr = ConsumerEnvironmentManager
                .initialse(consumerPropertyFileName);

        logger.debug("Connect to environment provider...");

        // connect to known environments if we are not yet connected
        if (!getConsumerEnvironment().getIsConnected())
        {
            ...
            if (connector != null)
            {
                ...
                else
                {
                    shutdown();
                    logAndThrowException(
                            "Failed to connect consumer to environment provider. See previous error log entries.");
                }
            }
            else
            {
                shutdown();
                logAndThrowException(
                        "Failed to retrieve a Environment Connector. See previoue error log entries for details.");
            }
        }
        ...
        {
            logger.info(
                    "Events and/or Delayed Responses are enabled. Startup async processing ...");
            if (!initAsyncProcessor())
            {
                shutdown();
                logAndThrowException(
                        "Failed to initialise async processing. See previoue error log entries for details.");
            }
        }
        else
        {
            logger.info(
                    "Events and Delayed Responses are not enabled. No event or delayed response processing has been started.");
        }
    ...
    private void shutdownConsumer()
    {
        ...
        {
            logger.debug("Remove Environment in Full = "
                    + envMgr.getEnvironmentInfo().getRemoveEnvOnShutdown());
    ...
    private boolean initAsyncProcessor()
    {
        ...
        for (AbstractEventConsumer<?> consumer : eventConsumers)
        {
            ...
            if (getConsumerEnvironment().getEventsEnabled())
            {
                addServices(allLocalQueueEventServices, consumer.getEventServices(),
                        consumer.getLocalConsumerQueue());
            }

            // Get CRUD services for this consumer
            if (getConsumerEnvironment().getDelayedEnabled())
            {
                addServices(allLocalQueueCRUDServices, consumer.getAllApprovedCRUDServices(),
                        consumer.getLocalConsumerQueue());
            }
        }
        ...
        try
        {
            ...
            queues = queueConnector.syncQueuesAtStartup(allLocalQueueEventServices,
                    allLocalQueueCRUDServices);
        }
        catch (Exception ex)
        {
        ...
        {
            logger.debug(
                    "Event Processing or Delayed Processing enabled and required. Queues and Configuration available...");
        ...
        {
            logger.debug(
                    "Event Processing enabled but no event subscriptions available. Don't start reading from message queues.");
        }
        return true;
    }
    ...
    private ExecutorService startRemoteMessageReaderThreads(QueueInfo queueInfo, int numThreads)
    {
        ...
        logger.debug("Total number of threads before starting message readers for "
                + remoteQueueName + " " + Thread.activeCount());
        ...
        for (int i = 0; i < numThreads; i++)
        {
            ...
            try
            {
                ...
                logger.debug("Start Remote Reader " + reader);
                service.execute(remoteReader);
            }
            catch (Exception ex)
            {
                logger.error("Failed to start message reader thread for : " + reader);
            }
        }
        logger.debug(
                numThreads + " " + remoteQueueName + " message readers initilaised and started.");
        logger.debug("Total number of threads after starting message readers for " + remoteQueueName
                + " " + Thread.activeCount());
        return service;
    }
    ...
    private void initialiseConsumers(AdvancedProperties adapterProps)
    {
        ...
        String basePackageName = makePackageName(
                adapterProps.getPropertyAsString("consumer.basePackageName", ""));
        for (String className : classList)
        {
            ...
            try
            {
                ...
                {
                    logger.debug("Added " + classObj.getClass().getSimpleName()
                            + " to eventConsumer list");
                    eventConsumers.add((AbstractEventConsumer<?>) classObj);
                }
                else if (classObj instanceof AbstractConsumer)
                {
                    logger.debug("Added " + classObj.getClass().getSimpleName()
                            + " to crudConsumer only list");
                    crudConsumers.add((AbstractConsumer) classObj);
                }
                else
                {
                    logger.error("Consumer class " + className
                            + " doesn't extend AbstractConsumer or AbstractEventConsumer. Cannot initialse the Consumer.");
                }
            }
            catch (Exception ex)
            {
                logger.error("Cannot create Consumer Class " + basePackageName + className + ": "
                        + ex.getMessage(), ex);
            }
        }
    }

    private void addServices(List<LocalQueueServiceInfo> localQueueServices,
            List<ServiceInfo> services, LocalConsumerQueue localQueue)
    {
