/*
 * RemoteMessageQueueReader.java Created: 08/05/2014
 *
...
/**
 * This class is the actual reader on the remote SIF Queue. It deals with all the logic that applies
 * to SIF Message Queues and how efficient reading from these queues can be achieved. It implements
 * the full queue reading process as defined in the SIF3 Specification. This class can be
 * multi-threaded as defined by the SIF3 Spec. Additionally this implementation will not fully
 * process the received message rather it will push it to a local queue where actual consumer
 * classes will be listening on. This allows the actual consumers to be either single threads or
 * multi-threads with out this class needing the knowledge how consumer are implemented.<br/>
 * <br/>
 * 
 * Note: It doesn't implement the full queue connector, just the "getMessage" part of the queue
 * connector.
 * 
 * @author Joerg Huber
 *
...
public class RemoteMessageQueueReader implements Runnable
{
     ...
     * parameter for the given session and consumer configuration.
     * 
     * @param queueInfo
            ...
            throws ServiceInvokationException
    {
        ...
        try
        {
            ...
            waitTime = Math.max(longToInt(getQueueInfo().getQueue().getWaitTime()),
                    getQueueInfo().getRemoteQueueConfig().getPollFrequency())
                    * CommonConstants.MILISEC;

            // Initialise message client. Only needs to be done once, so we do this here.
            client = new MessageClient(getConsumerEvnMgr(),
                    new URI(getQueueInfo().getQueue().getMessageURI()));
        }
        catch (Exception ex)
        {
    ...
    /*---------------------*/
    /*
     * This is the actual core methods. It connect to the SIF Message queue and starts reading
     * messages from it.
     */
    ...
    /*
     * Checks if an error is returned. If so it will log it, remove the lastMsg id and return true.
     * Otherwise false is returned. An error here means an issue with the actual message connector
     * (i.e. cannot connect, authentication etc). This is different to having retrieved an error
     * response message from the queue. The two differ in the following way: A proper error message
     * from a queue should have some mandatory HTTP headers. If they are there we can assume it is a
     * proper error message from a queue otherwise we assume it is an issue with the message client
     * and its connection. The fields to look out for are: - responseaction - relativeservicepath -
     * servicetype
     */
    private boolean isError(Response response)
    {
        if (response.hasError()) // An error message has been received.
        {
            if (StringUtils.isEmpty(
                    getHeaderValue(response, ResponseHeaderConstants.HDR_REL_SERVICE_PATH)))
            {
                // There is nothing we can do and it appears to be an error with the message client
                // and its comms with the queue (i.e Auth invalid)
            ...
            // We seem to have a relative service path which means the error is a genuine error
            // message from a queue to a delayed request.
            return false;
        }
        return false;
    }
    ...
    private void processMessage(Response response)
    {
        ...
        {
            logger.error(
                    "Message received with unknown message type (null or empty). Cannot process response:\n"
                            + response);
        ...
        {
            logger.error("Message received with unknown message type: '" + responseType
    ...
    private void processResponse(Response response)
    {
        try
        {
            ...
            LocalConsumerQueue localQueue = getQueueInfo().getLocalConsumerQueue(
                    responseInfo.getZone().getId(), responseInfo.getContext().getId(),
                    responseInfo.getServiceName(), responseInfo.getServiceType().name());

            if (localQueue == null) // There is a response for which there is no registered
                                    // consumer.
            {
                logger.info(
                        "Received a DELAYED Response for which there is no consumer registered. Discard the following RESPONSE:\n"
                                + response);
            }
        ...
        {
            logger.error(
                    "Error occured during the processing of a DELAYED RESPONSE message from the queue: "
                            + getQueueInfo().getQueue().getName()
                            + ". See previous error log entries for details: " + ex.getMessage(),
                    ex);
        }
    }
    ...
    private void processError(Response response)
    {
        try
        {
            ...
            LocalConsumerQueue localQueue = getQueueInfo().getLocalConsumerQueue(
                    errorInfo.getZone().getId(), errorInfo.getContext().getId(),
                    errorInfo.getServiceName(), errorInfo.getServiceType().name());

            if (localQueue == null) // There is an ERROR for which there is no registered consumer.
            {
                logger.info(
                        "Received a DELAYED Error for which there is no consumer registered. Discard the following Error:\n"
                                + response);
            }
            else // Create event object and send it to eventConsumer
            {
        ...
        {
            logger.error(
                    "Error occured during the processing of a DELAYED ERROR message from the queue: "
                            + getQueueInfo().getQueue().getName()
                            + ". See previous error log entries for details: " + ex.getMessage(),
                    ex);
        }
     ...
     */
    private void setResponseBaseData(DelayedBaseInfo baseInfo, Response response,
            MessageType messageType)
    {
        // First get all the info out from the relativeServicePath
        URIPathInfo pathInfo = new URIPathInfo(
                getHeaderValue(response, ResponseHeaderConstants.HDR_REL_SERVICE_PATH));
        ...
        // baseInfo.setZone(getZone(pathInfo.getZone() != null ? pathInfo.getZone().getId() :
        // null));
    ...
    private void processEvent(Response response)
    {
        try
        {
            ...
            LocalConsumerQueue localQueue = getQueueInfo().getLocalConsumerQueue(zone.getId(),
                    context.getId(), serviceName, serviceType);
            if (localQueue == null) // There is an event for which there is no registered consumer.
            {
                logger.info(
                        "Received an event for which there is no consumer registered. Discard the following Event:\n"
                                + response);
            }
            else // Create event object and send it to eventConsumer
            {
                ...
                metadata.setGeneratorID(
                        getHeaderValue(response, ResponseHeaderConstants.HDR_GENERATOR_ID));
        ...
        {
            logger.error(
                    "Error occured during the processing of a message from the queue: "
                            + getQueueInfo().getQueue().getName()
                            + ". See previous error log entries for details: " + ex.getMessage(),
                    ex);
        }
    ...
    private UpdateType getUpdateType(Response response)
    {
        ...
        {
            logger.error(
                    "Received an update event with no update type set in the response header:\n"
                            + response);
            return null;
        }
    }
    ...
    /*
     * Utility method to get the SIF Zone Object based on zone ID. If zoneID is empty then the
     * Default Zone is returned.
     */
    ...
    /*
     * Utility method to get the SIF Context Object based on context ID. If contextID is empty then
    ...
    {
        this.lastMsgeID = response.getHdrProperties()
                .getHeaderProperty(ResponseHeaderConstants.HDR_MESSAGE_ID);
    }
